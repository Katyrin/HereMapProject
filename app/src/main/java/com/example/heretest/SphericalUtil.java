...
public class SphericalUtil {
    ...
    public static double computeHeading(GeoCoordinates from, GeoCoordinates to) {
    ...
    public static GeoCoordinates computeOffset(GeoCoordinates from, double distance, double heading) {
        distance /= 6371009.0D;
        ...
        return new GeoCoordinates(Math.toDegrees(Math.asin(sinLat)), Math.toDegrees(fromLng + dLng));
    }

    public static GeoCoordinates computeOffsetOrigin(GeoCoordinates to, double distance, double heading) {
        heading = Math.toRadians(heading);
        distance /= 6371009.0D;
        ...
        if (discriminant < 0.0D) {
            ...
            if (fromLatRadians >= -1.5707963267948966D && fromLatRadians <= 1.5707963267948966D) {
                ...
                return new GeoCoordinates(Math.toDegrees(fromLatRadians), Math.toDegrees(fromLngRadians));
            } else {
                return null;
            }
        }
    ...
    public static GeoCoordinates interpolate(GeoCoordinates from, GeoCoordinates to, double fraction) {
        double fromLat = Math.toRadians(from.latitude);
        double fromLng = Math.toRadians(from.longitude);
        double toLat = Math.toRadians(to.latitude);
        double toLng = Math.toRadians(to.longitude);
        ...
        if (sinAngle < 1.0E-6D) {
            ...
            return new GeoCoordinates(Math.toDegrees(lat), Math.toDegrees(lng));
        }
    }
    ...
    static double computeAngleBetween(GeoCoordinates from, GeoCoordinates to) {
    ...
    public static double computeDistanceBetween(GeoCoordinates from, GeoCoordinates to) {
        return computeAngleBetween(from, to) * 6371009.0D;
    }

    public static double computeLength(List<GeoCoordinates> path) {
        if (path.size() < 2) {
            return 0.0D;
        } else {
            double length = 0.0D;
    ...
    public static double computeArea(List<GeoCoordinates> path) {
        return Math.abs(computeSignedArea(path));
    }

    public static double computeSignedArea(List<GeoCoordinates> path) {
        return computeSignedArea(path, 6371009.0D);
    }

    static double computeSignedArea(List<GeoCoordinates> path, double radius) {
        int size = path.size();
        if (size < 3) {
            return 0.0D;
        } else {
