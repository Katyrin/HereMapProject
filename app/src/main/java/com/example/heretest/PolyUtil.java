...
public class PolyUtil {
    ...
    public static boolean containsLocation(GeoCoordinates point, List<GeoCoordinates> polygon, boolean geodesic) {
        return containsLocation(point.latitude, point.longitude, polygon, geodesic);
    }

    public static boolean containsLocation(double latitude, double longitude, List<GeoCoordinates> polygon, boolean geodesic) {
    ...
    public static boolean isLocationOnEdge(GeoCoordinates point, List<GeoCoordinates> polygon, boolean geodesic, double tolerance) {
        return isLocationOnEdgeOrPath(point, polygon, true, geodesic, tolerance);
    }

    public static boolean isLocationOnEdge(GeoCoordinates point, List<GeoCoordinates> polygon, boolean geodesic) {
        return isLocationOnEdge(point, polygon, geodesic, 0.1D);
    }

    public static boolean isLocationOnPath(GeoCoordinates point, List<GeoCoordinates> polyline, boolean geodesic, double tolerance) {
        return isLocationOnEdgeOrPath(point, polyline, false, geodesic, tolerance);
    }

    public static boolean isLocationOnPath(GeoCoordinates point, List<GeoCoordinates> polyline, boolean geodesic) {
        return isLocationOnPath(point, polyline, geodesic, 0.1D);
    }

    private static boolean isLocationOnEdgeOrPath(GeoCoordinates point, List<GeoCoordinates> poly, boolean closed, boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return false;
        } else {
    ...
    public static List<GeoCoordinates> simplify(List<GeoCoordinates> poly, double tolerance) {
        int n = poly.size();
        if (n < 1) {
    ...
    public static double distanceToLine(GeoCoordinates p, GeoCoordinates start, GeoCoordinates end) {
        if (start.equals(end)) {
            ...
            if (u <= 0.0D) {
                ...
                GeoCoordinates sa = new GeoCoordinates(p.latitude - start.latitude, p.longitude - start.longitude);
                GeoCoordinates sb = new GeoCoordinates(u * (end.latitude - start.latitude), u * (end.longitude - start.longitude));
                return SphericalUtil.computeDistanceBetween(sa, sb);
            }
        }
    }

    public static List<GeoCoordinates> decode(String encodedPath) {
        int len = encodedPath.length();
        List<GeoCoordinates> path = new ArrayList();
        int index = 0;
        int lat = 0;
        int lng = 0;
    ...
    public static String encode(List<GeoCoordinates> path) {
        long lastLat = 0L;
        long lastLng = 0L;
